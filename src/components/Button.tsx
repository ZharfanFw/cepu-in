...
export default function Button({href, label, icon="", primary=false, showIcon=false}: ButtonData) {
    return (
        <Link href={href} scroll={true} className={`flex gap-2 ${primary ? "bg-(--color-normal) hover:bg-(--color-normal-hover) text-white" : "bg-white hover:bg-(--color-light) text-(--color-normal) border border-(--color-normal)"} px-4 py-2 rounded-4xl transition-all duration-150 justify-center items-center font-medium text-[14.5px] max-sm:text-[12px]`}>
